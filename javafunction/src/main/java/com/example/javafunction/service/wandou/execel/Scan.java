        ...
        for (File f : file.listFiles()) {
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(f))) {
                while ((line = br.readLine()) != null) {
                    ...
                    String key = findKey(line, ketSet);
                    if(StringUtils.isNotBlank(key)) {
//                        System.out.println();
//                        out.println(String.format("  %s :  %s ",key,describe(key,lineArr[maps.get(key)-1])));
                        List<Object> data=new ArrayList<Object>();
                        data.add(key);
                        int index = maps.get(key) - 1;
                        if(lineArr.length>index) {
//                            data.add(describe(key,lineArr.length>index?lineArr[maps.get(key)-1]:""));
                            fileValues.put(key,describe(key,lineArr.length>index?lineArr[maps.get(key)-1]:""));
                        }else {
                            System.out.println("error column key "+ key + " file "+ f.getName());
                        }
                    }
...
//        out.close();
    ...
    private static String findKey(String lineArr, Set<String> keys) {
        for (String key : keys) {
            if(lineArr.contains(key)) {
                return key;
            }
        }
        return null;
